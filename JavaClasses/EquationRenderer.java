...
public class EquationRenderer {
      ...
      for (int x = 0; x < graphPixel.length; x++) {
        for (int y = 0; y < graphPixel[0].length; y++) {
          ...
          if (graphPixel[x][y].graph && !graphPixel[x][y].visited) {
            connectNeighbours(x,y,linePoints, graphPixel, res, midpoint, zoom);
          } // end of if
        }
      }
      if (isCancelled()) {
  ...
  public void connectNeighbours(int x, int y, ArrayList<TwoDVec<TwoDVec<Double>>> linePoints, GraphPixel[][] graphPixel, TwoDVec<Double> res, TwoDVec<Double> mid, TwoDVec<Double> zoom) {   //checks the eight neighbouring pixels for if they're part of the graph
    double deltaX = res.x/2;
    double deltaY = res.y/2;
    if (graphPixel[x+1][y+1].graph && !graphPixel[x+1][y+1].visited) {
      TwoDVec<TwoDVec<Double>> line = new TwoDVec<TwoDVec<Double>>(renderValues.screenCoordDoubleToRealCoord(new TwoDVec<Double>((double)x-deltaX,(double)y-deltaY),mid,zoom),renderValues.screenCoordDoubleToRealCoord(new TwoDVec<Double>((double)x+1-deltaX,(double)y+1-deltaY),mid,zoom));
    ...
    if (graphPixel[x+1][y].graph && !graphPixel[x+1][y].visited) {
      TwoDVec<TwoDVec<Double>> line = new TwoDVec<TwoDVec<Double>>(renderValues.screenCoordDoubleToRealCoord(new TwoDVec<Double>((double)x-deltaX,(double)y-deltaY),mid,zoom),renderValues.screenCoordDoubleToRealCoord(new TwoDVec<Double>((double)x+1-deltaX,(double)y-deltaY),mid,zoom));
      linePoints.add(line);
    } // end of if
    if (graphPixel[x][y+1].graph && !graphPixel[x][y+1].visited) {
      TwoDVec<TwoDVec<Double>> line = new TwoDVec<TwoDVec<Double>>(renderValues.screenCoordDoubleToRealCoord(new TwoDVec<Double>((double)x-deltaX,(double)y-deltaY),mid,zoom),renderValues.screenCoordDoubleToRealCoord(new TwoDVec<Double>((double)x-deltaX,(double)y+1-deltaY),mid,zoom));
      linePoints.add(line);
    } // end of if
    if (graphPixel[x-1][y].graph && !graphPixel[x-1][y].visited) {
      TwoDVec<TwoDVec<Double>> line = new TwoDVec<TwoDVec<Double>>(renderValues.screenCoordDoubleToRealCoord(new TwoDVec<Double>((double)x-deltaX,(double)y-deltaY),mid,zoom),renderValues.screenCoordDoubleToRealCoord(new TwoDVec<Double>((double)x-1-deltaX,(double)y-deltaY),mid,zoom));
  ...
  public ArrayList<ArrayList<TwoDVec<TwoDVec<Double>>>> calculateEquationsLinePoints(ArrayList<EquationTree> equations,Variable[] existingVariables, EquationTree[] existingFunctions) {
    ...
    double xZoom = 0.5;
    double yZoom = 0.5;
    double xMove = renderValues.resolution.x/2;
    double yMove = renderValues.resolution.y/2;
    if ((double)lastZoom.x != (double)renderValues.zoom.x || (double)lastZoom.y != (double)renderValues.zoom.y) {
      ...
      if (Math.abs((double)lastPos.x - (double)renderValues.midpoint.x) > xMove || Math.abs((double)lastPos.y - (double)renderValues.midpoint.y) > yMove) {
