...
public class RenderImage {
    ...
    public RenderImage convertToDifferentCoord(TwoDVec<Double> midpoint, TwoDVec<Double> zoom, TwoDVec<Double> viewSize) {
        TwoDVec<Double> imagePos = new TwoDVec<Double>(-viewSize.x / 2 + midpoint.x - renderPos.x,-viewSize.y / 2 + midpoint.y - renderPos.y);
        TwoDVec<Double> zoomFacs = new TwoDVec<Double>(1/renderZoom.x,1/renderZoom.y);
        TwoDVec<Double> imageSize = new TwoDVec<Double>(viewSize.x / (zoom.x * zoomFacs.x),viewSize.y / (zoom.y * zoomFacs.y));
